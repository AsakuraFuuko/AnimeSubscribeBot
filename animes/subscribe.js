                ...
                for (let anime of result.animes) {
                    ...
                    if (anime.results.length > 0) {
                        for (let obj of Utils.UniqueBy(anime.results.map((result) => result.data), (item) => item.title)) {
                            this.db.episodes.addEpisode(result.user.user_id, anime.title, obj.title, obj.torrent)
                        }
                        if (result.user.status) {
                            self.tgbot.telegram.sendMessage(result.user.user_id, text, {parse_mode: 'HTML'}).then(() =>
